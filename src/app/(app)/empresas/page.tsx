...
export default function EmpresasPage() {
  ...
  const [activeTab, setActiveTab] = useState<'dados' | 'dossie' | 'socios'>('dados')
  ...
  async function fetchData() {
    ...
    const { data: byId } = await supabase
      .from('empresas').select('*').eq('org_id', orgId).order('razao_social')
    if (byId && byId.length > 0) { setEmpresas(byId); setLoading(false); return }
    const { data: byName } = await supabase
      .from('empresas').select('*').eq('organizacao', orgName).order('razao_social')
    setEmpresas(byName || [])
    setLoading(false)
  }
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (!error) {
      fetchData(); setIsModalOpen(false); setFormData(initialForm); toast.success('Empresa salva!')
  ...
  function toggleDetail(emp: any, tab: 'dados' | 'dossie' | 'socios' = 'dados') {
    if (detailId === emp.id && activeTab === tab) { setDetailId(null); return }
    setDetailId(emp.id)
    setActiveTab(tab)
  }
              ...
              {empresasFiltradas.map(emp => {
                ...
                return (
                  <>
                    <tr
                      key={emp.id}
                      onClick={() => toggleDetail(emp, 'dados')}
                      className={`border-b border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors ${isOpen ? 'bg-yellow-50 hover:bg-yellow-50' : ''}`}
                    >
                      {/* Status */}
                      ...
                      <td className="px-4 py-3 hidden lg:table-cell">
                        {tag && (
                          <span className="text-[9px] font-black px-2 py-0.5 rounded bg-slate-100 text-slate-600 uppercase">{tag}</span>
                        )}
                      </td>
                      ...
                      <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          <button
                            onClick={() => toggleDetail(emp, 'dossie')}
                            title="Dossiê CNPJ"
                            className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase transition-all ${activeTab === 'dossie' && isOpen ? 'bg-black text-yellow-400' : 'bg-slate-100 text-slate-600 hover:bg-yellow-100 hover:text-yellow-700'}`}
                          >
                            Dossiê
                          </button>
                          <button
                            onClick={() => { setFormData(emp); setIsModalOpen(true) }}
                            title="Editar"
                          ...
                          <button
                            ...
                            title="Excluir"
                            className="w-7 h-7 rounded-lg bg-slate-100 hover:bg-red-100 text-slate-500 hover:text-red-600 flex items-center justify-center text-sm transition-all"
                          >✕</button>
                        </div>
                      </td>
                    </tr>
                    ...
                    {/* ── Painel inline ── */}
                    {isOpen && (
                      <tr key={`${emp.id}-detail`} className="bg-slate-50">
                        <td colSpan={6} className="px-4 md:px-6 py-4 border-b border-slate-200">
                          ...
                          <div className="flex gap-1 mb-4 border-b border-slate-200 overflow-x-auto">
                            {([
                              ...
                              <button
                                ...
                                className={`px-4 py-2 text-xs font-black uppercase whitespace-nowrap border-b-2 transition-all ${
                                  activeTab === tab
                          ...
                          {activeTab === 'dados' && (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 text-xs">
                              {[
                                ...
                                <div key={label} className={extra as string}>
                              ...
                              {emp.cnaes_secundarios?.length > 0 && (
                                <div className="col-span-2 sm:col-span-3 md:col-span-4">
                                  <p className="text-[9px] text-slate-400 font-black uppercase tracking-wide mb-1">CNAEs Secundários</p>
                                  <div className="flex flex-wrap gap-1">
                                    {emp.cnaes_secundarios.map((c: any, i: number) => (
                                      <span key={i} className="text-[9px] bg-white border border-slate-200 rounded px-2 py-0.5 font-mono">{c.codigo} — {c.descricao}</span>
                                    ))}
                                  </div>
                                </div>
                              )}
                              ...
                              <div className="col-span-2 sm:col-span-3 md:col-span-4 pt-2">
                                <button
                                  onClick={() => { setFormData(emp); setIsModalOpen(true) }}
                                  className="bg-black text-yellow-400 px-5 py-2 rounded-xl text-xs font-black uppercase hover:bg-slate-800 transition-all"
                                >
                                  ✎ Editar Dados
                                </button>
                              </div>
                          ...
                          {/* Tab: Dossiê CNPJ (cartão oficial) */}
                          {activeTab === 'dossie' && (
                            <div className="bg-white border-2 border-black rounded-xl overflow-hidden max-w-2xl">
                              ...
                              <div className="grid grid-cols-2 sm:grid-cols-4 border-b-2 border-black text-[10px]">
                                <div className="p-2 border-r-2 border-black">
                                  ...
                                  <p className="font-mono font-bold">{emp.cnpj}</p>
                                </div>
                                <div className="p-2 border-r-2 border-black col-span-2 flex items-center justify-center font-black text-[10px] uppercase text-center">
                                  Comprovante de Situação Cadastral
                                </div>
                                <div className="p-2">
                                  <p className="text-[7px] text-slate-400 font-bold uppercase">ABERTURA</p>
                                  <p className="font-bold">{emp.data_abertura?.split('-').reverse().join('/')}</p>
                                </div>
                              </div>

                              <div className="p-0 space-y-px bg-black">
          ...
          <div className="bg-white w-full max-w-lg rounded-2xl p-8 shadow-2xl relative">
            ...
            <div className="flex gap-2 mb-6">
              ...
              <button
                ...
                className="bg-black text-yellow-400 px-6 rounded-xl font-bold text-xs uppercase disabled:opacity-50"
              >
                {isConsulting ? '...' : 'BUSCAR'}
              </button>
            </div>
