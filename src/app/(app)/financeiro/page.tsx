            ...
            {lancamentos.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50 transition-colors group text-left">
                ...
                <td className="px-6 py-4 text-right">
                  <button onClick={() => handleDelete(item.id)} className="p-2 text-slate-300 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all" title="Excluir">
                    <IconTrash />
                  </button>
                </td>
              </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-6 text-left">
          <div className="bg-white w-full max-w-lg rounded-2xl p-8 shadow-2xl border border-slate-200">
            <h2 className="text-xl font-bold text-slate-900 mb-6 text-left">Novo Lançamento Financeiro</h2>
            <form onSubmit={handleSave} className="space-y-5 text-left">
              ...
              <div className="grid grid-cols-2 gap-4 text-left">
                ...
                <div className="space-y-1 text-left">
                  ...
                  <input type="number" step="0.01" required className="w-full bg-slate-50 border border-slate-200 rounded-lg p-3 text-sm font-bold" onChange={(e) => setFormData({...formData, valor: e.target.value})} />
                </div>
              </div>
              <div className="space-y-1 text-left">
